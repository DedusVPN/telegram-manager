...
from database import Account, Task
    ...
    try:
        ...
        async with db.session_maker() as session:
            result = await session.execute(
                select(Account).where(Account.id.in_(data['selected']))
            )
            accounts = result.scalars().all()
        ...
        import asyncio
        asyncio.create_task(message_sender.execute_task(
            task.id, accounts, data['target'], data['text'],
            data['repeat'], data['interval'], delay
        ))
